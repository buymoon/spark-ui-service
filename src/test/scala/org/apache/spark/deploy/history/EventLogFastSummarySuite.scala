...
class EventLogFastSummarySuite extends AnyFunSuite {
  ...
  test("fast summary ignores malformed final line for in-progress event logs") {
    val tempDir = Files.createTempDirectory("fast-summary-inprogress-suite")
    val eventLog = tempDir.resolve("local-inprogress.inprogress")
    val uimetaDir = tempDir.resolve("uimeta")
    Files.createDirectories(uimetaDir)

    val lines = Seq(
      """{"Event":"SparkListenerApplicationStart","App Name":"In Progress App","App ID":"local-inprogress","Timestamp":1000,"User":"alice"}""",
      """{"Event":"SparkListenerJobStart","Job ID":1,"Submission Time":2000,"Stage Infos":[],"Stage IDs":[],"Properties":{}}""",
      """{"Event":"SparkListenerStageSubmitted","Stage Info":{"Stage ID":1,"Stage Attempt ID":0,"Stage Name":"active stage","Number of Tasks":2,"RDD Info":[],"Parent IDs":[],"Details":"running","Submission Time":2100,"Accumulables":[],"Resource Profile Id":0},"Properties":{}}""",
      """{"Event":"SparkListenerStageCompleted","Stage Info":{"Stage ID":1,"Stage Attempt ID":0,"Stage Name":"active stage","Number of Tasks":2,"RDD Info":[],"Parent IDs":[],"Details":"running","Submission Time":2100,"Completion Time":2600,"Accumulables":[],"Resource Profile Id":0}}""",
      """{"Event":"SparkListenerJobEnd","Job ID":1,"Completion Time":3000,"Job Result":{"Result":"JobSucceeded"}}""",
    ...
    assert(result.appId == "local-inprogress")
    val store = loadStore(result.uimetaPath.toString)
    val stages = store.view(classOf[StageDataWrapper]).iterator()
    assert(stages.hasNext)
    assert(stages.next().info.stageId == 1)
  }
