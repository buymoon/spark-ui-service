...
private[history] object FastEventLogSummary {
  ...
  private def parse(eventLog: String, hadoopConf: Configuration): Summary = {
    ...
    EventLogFileReader(fs, path) match {
      case Some(reader) =>
        reader.listEventLogFiles.foreach(status => scanFile(status, fs, state))
      case None =>
        scanPath(path, fs, state)
    }

    state.toSummary()
  }

  private def scanFile(status: FileStatus, fs: FileSystem, state: SummaryState): Unit = {
    scanPath(status.getPath, fs, state)
  }

  private def scanPath(path: Path, fs: FileSystem, state: SummaryState): Unit = {
  ...
  private def handleLine(line: String, state: SummaryState): Unit = {
